...
import { useEditColorMutation } from "../../redux/features/color/colorApi";

const EditColor = ({ closeModal, data }) => {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const [updateColor, { isLoading }] = useEditColorMutation();
  ...
  const HandleColor = async (value) => {
    const res = await updateColor({ id: data?.id, value });
    console.log(res);
    if (res?.data) {
