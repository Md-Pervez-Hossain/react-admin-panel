...
import { useGetBulkSmsStatusDetailsQuery } from "../../../redux/features/status/bulkSmsStatus/bulkSmsStatusApi";
import Container from "../../../share/ui/Container/Container";

const BulkSmsStatusDetails = () => {
  const { id } = useParams();
  const { data: bulkSmsHistoryDetails } = useGetBulkSmsStatusDetailsQuery(id);
              ...
              {bulkSmsHistoryDetails?.numbers?.map((data) => {
                ...
                return (
                  <tr key={data?.id}>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm bg-red-200 text-red-500 rounded-md text-center">
                      {data?.status}
                    </td>
                  </tr>
                );
...
export default BulkSmsStatusDetails;
